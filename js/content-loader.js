 ...
 * Loads content from JS data object or JSON files and updates the HTML
 */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sections = ['header', 'about', 'services', 'process', 'gallery', 'stats', 'testimonials', 'contact'];
...
async function loadContent(sectionId) {
  try {
    ...
    // First try to get data from the window.siteContent object
    if (window.siteContent && window.siteContent[sectionId]) {
      console.log(`Using embedded data for ${sectionId}`);
      data = window.siteContent[sectionId];
    } else {
      // Fallback to fetching from JSON file
      console.log(`Fetching data for ${sectionId} from JSON`);
